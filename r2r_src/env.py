...
class R2RBatch():
    ...
    def _load_nav_graphs(self):
        ...
        for scan, G in self.graphs.items(): # compute all shortest paths
            ...
            self.dists[scan] = dict(nx.all_pairs_dijkstra_path_length(G))
            self.discrete_dists[scan] = dict(nx.shortest_path_length(G))
        self.distances = {}
        for scan, G in self.graphs.items(): # compute all shortest paths
    ...
    def _update_obs(self, obs, idx, i):
        feature, state = self.env.getStates(idx)
        item = self.batch[idx]
        ...
        try:
            cur_viewpoint_id = state.location.viewpointId
            path_idx = item['path'].index(cur_viewpoint_id)
            if path_idx < len(item['path']) - 1:
                teacher = item['path'][path_idx + 1]
            else:
                teacher = item['path'][-1]
        except:
            teacher = self._shortest_path_action(state, item['path'][-1])
        # print(teacher == self._shortest_path_action(state, item['path'][-1]))
        obs[i] = ({
        ...
        for i, (feature, state) in enumerate(self.env.getStates()):
            ...
            if self.dataset == 'R2R':
                ...
                for bbox in item['bboxes']:
                    image_ids.append(bbox['image_id'])
                    obj_headings.append(bbox['heading'])
                    obj_elevations.append(bbox['elevation'])
                    if args.compute_bbox:
                        # x1, y1, x2, y2
                        bboxes_2d.append([bbox['mouse_left_top']['x'], bbox['mouse_left_top']['y'],
                                        bbox['mouse_right_bottom']['x'], bbox['mouse_right_bottom']['y']])
                        offset_headings.append(bbox['offset_heading'])
                        offset_elevations.append(bbox['offset_elevation'])
                        num_headings.append(bbox['num_heading'])
                        num_elevations.append(bbox['num_elevation'])
                        # old_headings.append(bbox['old_heading'])
                        # old_elevations.append(bbox['old_elevation'])
                ob = {
                    'instr_id': item['instr_id'],
                    'scan': state.scanId,
                    'viewpoint': state.location.viewpointId,
                    ...
                    'teacher': self._shortest_path_action(state, item['path'][-1]),
                    'path_id': item['path_id'],
                    'obj_headings': obj_headings,
                    'obj_elevations': obj_elevations,
                    'image_ids': image_ids
                }
                if args.compute_bbox:
                    ob['bboxes_2d'] = bboxes_2d
                    ob['offset_headings'] = offset_headings
                    ob['offset_elevations'] = offset_elevations
                    ob['num_headings'] = num_headings
                    ob['num_elevations'] = num_elevations
                    # ob['old_headings'] = old_headings
                    # ob['old_elevations'] = old_elevations
                obs.append(ob)
