...
class OrientationIndicator(nn.Module):
    def __init__(self):
        super(OrientationIndicator, self).__init__()
...
class BboxIndicator(nn.Module):
    def __init__(self):
        super(BboxIndicator, self).__init__()
        hidden_size = args.rnn_dim + 2
        self.fc1 = nn.Linear(hidden_size, hidden_size)
        self.relu1 = nn.LeakyReLU()
...
class HeadingNumIndicator(nn.Module):
    def __init__(self):
        super(HeadingNumIndicator, self).__init__()
        hidden_size = args.rnn_dim + 2
        self.fc1 = nn.Linear(hidden_size, hidden_size)
        self.relu1 = nn.LeakyReLU()
        self.fc2 = nn.Linear(hidden_size, args.heading_class_num)
        self.sigmoid = nn.Sigmoid()
...
class ElevationNumIndicator(nn.Module):
    def __init__(self):
        super(ElevationNumIndicator, self).__init__()
        hidden_size = args.rnn_dim + 2
        self.fc1 = nn.Linear(hidden_size, hidden_size)
        self.relu1 = nn.LeakyReLU()
        self.fc2 = nn.Linear(hidden_size, args.elevation_class_num)
        self.sigmoid = nn.Sigmoid()
...
class GraphConvolution(nn.Module):
    ...
    def forward(self, adj, input):
        ...
        support = torch.bmm(input, self.weight.unsqueeze(0).expand(batch_size, *weight_shape))
...
# class GNN(nn.Module):
#     def __init__(self, cin, cout, dropout_ratio=None):
#         super(GNN, self).__init__()
#         self.gc1 = GraphConvolution(cin, cout)
...
class SoftDotAttention(nn.Module):
    ...
    def __init__(self, query_dim, ctx_dim, hidden_dim=None):
        '''Initialize layer.'''
        ...
        if hidden_dim is None:
            ...
            self.linear_in = nn.Linear(query_dim, hidden_dim, bias=False)
            self.linear_in2 = nn.Linear(ctx_dim, hidden_dim, bias=False)
        self.sm = nn.Softmax()
...
class VisEncoder(nn.Module):
    def __init__(self, hidden_size, feature_size, gnn_size, dropout_ratio):
        super(VisEncoder, self).__init__()
        self.drop = nn.Dropout(p=dropout_ratio)
        self.drop_env = nn.Dropout(p=args.featdropout)
        self.feat_att_layer = SoftDotAttention(hidden_size, feature_size, gnn_size)
        # self.fc = nn.Linear(feature_size, gnn_size)
    
    def forward(self, feature, ctx, already_dropfeat=False):
        ...
        ctx_drop = self.drop(ctx)
        attn_feat, _ = self.feat_att_layer(ctx_drop[:,0,:], feature, output_tilde=False)
        # return self.fc(attn_feat)
        return attn_feat
    
class CanEncoder(nn.Module):
    def __init__(self, hidden_size, feature_size, dropout_ratio):
        super(CanEncoder, self).__init__()
        # self.drop = nn.Dropout(p=dropout_ratio)
        self.drop_env = nn.Dropout(p=args.featdropout)
        self.fc = nn.Linear(feature_size, hidden_size)
    
    def forward(self, feature, already_dropfeat=False):
        if not already_dropfeat:
            # Dropout the raw feature as a common regularization
            # Do not drop the last args.angle_feat_size (position feat)
            feature[..., :-args.angle_feat_size] = self.drop_env(feature[..., :-args.angle_feat_size])
...
class GraphDecoderLSTM(nn.Module):
    ''' An unrolled LSTM with attention over instructions for decoding navigation actions. '''

    def __init__(self, embedding_size, gnn_size, rnn_size, dropout_ratio):
        super(GraphDecoderLSTM, self).__init__()
        self.embedding_size = embedding_size
        ...
        self.drop = nn.Dropout(p=dropout_ratio)
        self.lstm = nn.LSTMCell(embedding_size + gnn_size, rnn_size)
        self.attention_layer = SoftDotAttention(rnn_size, rnn_size)
        self.candidate_att_layer = SoftDotAttention(rnn_size, gnn_size)

    def forward(self, action, gnn_feature, cand_feat, prev_h1, h_t, c_t, ctx, ctx_mask=None):
        action_embeds = self.embedding(action)
        action_embeds = self.drop(action_embeds)

        concat_input = torch.cat((action_embeds.cuda(), gnn_feature), dim=1)
        h_1, c_1 = self.lstm(concat_input, (prev_h1, c_t))

        h_1_drop = self.drop(h_1)
        h_tilde, alpha = self.attention_layer(h_1_drop, ctx, ctx_mask) # if out of memory, delete this
