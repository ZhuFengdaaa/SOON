...
def load_datasets(splits, dataset='R2R'):
    ...
    PATH_IDS = {'train': 0, 'val_seen_instrs': 1, 'val_unseen_instrs': 2, 'val_unseen_house': 3, "test": 4}
    for split in splits:
        ...
        if "/" not in split:
            if dataset == 'R2R':
                if args.upload:
                    load_dir = get_sync_dir(os.path.join(args.upload_path,'tasks/R2R/data/R2R_%s.json' % split))
                    print(load_dir)
                else:
                    load_dir = 'tasks/R2R/data/R2R_%s.json' % split
                with open(load_dir) as f:
                    new_data = json.load(f)
            elif dataset == 'SOON':
                path_num = 0
                if args.upload:
                    load_dir = get_sync_dir(os.path.join(args.upload_path, 'tasks/SOON/%s.json' % split))
                else:
                    load_dir = 'tasks/SOON/%s.json' % split
                print(load_dir)
                with open(load_dir) as f:
                    ...
                    for instr in read_data:
                        ...
                        for path in paths:
                            ...
                            if args.one_image:
                                tmp_instr['path'] = [path[-1]]
                            else:
                                tmp_instr['path'] = path
                            tmp_instr['scan'] = scan
                            tmp_instr['path_id'] = str(PATH_IDS[split]) + '-' + str(path_num)
                            path_num += 1
                            # normalize
                            new_bboxes = []
                            for bbox in instr['bboxes']:
                                new_bbox = copy.deepcopy(bbox)
                                new_bbox['old_heading'] = new_bbox['heading'] / (2 * math.pi)
                                new_bbox['old_elevation'] = (new_bbox['elevation'] + math.pi) / (2 * math.pi)
                                ...
                                if args.compute_bbox:
                                    heading_inter = int(360 / args.heading_class_num)
                                    elevation_inter = int(360 / args.elevation_class_num)
                                    heading_angle = new_bbox['old_heading'] * 2 * 180
                                    elevation_angle = new_bbox['old_elevation'] * 2 * 180
                                    new_bbox['offset_heading'] = heading_angle % heading_inter
                                    new_bbox['offset_elevation'] = elevation_angle % elevation_inter
                                    new_bbox['num_heading'] = int(heading_angle // heading_inter)
                                    new_bbox['num_elevation'] = int(elevation_angle // elevation_inter)
                                new_bboxes.append(new_bbox)
                            tmp_instr['bboxes'] = new_bboxes
                            new_data.append(tmp_instr)
        else:
...
def build_vocab(splits=['train'], min_count=5, start_vocab=base_vocab, dataset='R2R'):
    ''' Build a vocab, starting with base vocab containing a few useful tokens. '''
    count = Counter()
    t = Tokenizer()
    data = load_datasets(splits, dataset=dataset)
    for item in data:
        for instr in item['instructions']:
            if dataset == 'R2R':
                ...
                last_instr = instr[4]
                if not contain_zh(last_instr):
                    count.update(t.split_sentence(last_instr))
                else:
                    print(last_instr)
